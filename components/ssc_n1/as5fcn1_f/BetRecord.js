...
export const As5fcn1_fBetRecord = () => {
    ...
    return (
        <LayoutContainer>
            <div className="inside_header">
                <a onClick={handleBack} className="ffc_back">
                    <span>
                        <img src={image("angle.png")} alt="" />
                    </span>
                    返回
                </a>
                <div className="ffc_tit">{showDetail ? "注单详情" : "投注记录"}</div>
            </div>
            <article className="ffc_my_bets">
                <p className="bets_note">※ 点选可看详情</p>
                <div id="bets">
                    <dl>
                        <dt>
                            <div>期号</div>
...
const Detail = ({ order }) => {
    ...
    return (
        <div className="detail">
            ...
            {isSuccess &&
                list.map(({ id, bet_type, bet_detail, bets, bet_currency_amount, currency, status, status_text, win_currency_amount, odds }) => (
                    <dl key={id}>
                        <dd className="tit">
                            <div>玩法名称</div>
                            <div>{bet_type}</div>
                        ...
                        <dd>
                            <div>投注内容</div>
                            <div className="detail-content">{bet_detail}</div>
                        </dd>
                        <dd>
                            <div>投注金额</div>
                            <div>
