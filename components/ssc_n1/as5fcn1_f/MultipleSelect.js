...
export const As5fcn1_fMultipleSelect = forwardRef((props, ref) => {
        ...
        <div ref={ref} className={`bot_menu_popup active`}>
            <div className="bot_menu_btn">
                {MULTIPLE_MAP.map((value) => {
                    ...
                    return (
                        <div id={`multiple-select-${value}`} key={value} className={`${isActive ? "active" : ""}`} onClick={() => multipleHandle.change(value)}>
                            {value}
                        </div>
                    );
                })}
            ...
            <div className="bot_menu_input">
                ...
                <input aria-label="multiple-input" value={multiple} type="tel" maxLength={5} onChange={multipleHandle.input} />
