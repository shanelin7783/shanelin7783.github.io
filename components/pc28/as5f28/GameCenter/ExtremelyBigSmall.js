...
import { EXTREMELY_BIG_SMALL_LIST } from "@core/constants/pc28/oddConfig";

export const ExtremelyBigSmall = () => {
    ...
    return (
        <section className="play_number">
            <div className="number_con">
                {EXTREMELY_BIG_SMALL_LIST.map((bet_type) => {
                    const { odds = 0, bet_type_msg = "" } = bet_odds_config[bet_type] || {};
                    const isActive = itemList.has(bet_type);
                    return (
                        <div key={bet_type} className={`number_box4 ${isActive ? "active" : ""}`} onClick={onToggleItem(bet_type)}>
