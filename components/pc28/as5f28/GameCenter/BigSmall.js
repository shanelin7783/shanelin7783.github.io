...
import { BIG_SMALL_LIST } from "@core/constants/pc28/oddConfig";
...
export const BigSmall = () => {
    ...
    return (
        <section className="play_number">
            {chunk(BIG_SMALL_LIST, 4).map((block, index) => (
                <div key={index} className="number_con">
                    {block.map((bet_type) => {
                        const { odds = 0, bet_type_msg = "" } = bet_odds_config[bet_type] || {};
                        const isActive = itemList.has(bet_type);
                        return (
                            <div key={bet_type} className={`number_box3 ${isActive ? "active" : ""}`} onClick={onToggleItem(bet_type)}>
